...
- ✅ `flag` 局部变量未初始化隐患已加固（2026-06-22 修复）：`src/updatewtd_shallow.jl:35` 显式 `flag = 0` 初始化；L39-L41 注释确认"每轮迭代开始时将 flag 重置为 0；若 kwt <= 1 将跳过 `flag = 1` 赋值分支，此时 `flag` 保持初始 0，不会残留上一轮的值（避免污染当前迭代）"。
