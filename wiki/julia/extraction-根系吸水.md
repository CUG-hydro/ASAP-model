...
1. ✅ `kroot = k - 1` 语义已澄清（2026-06-22 修复）：`src/extraction.jl:65-L70` 注释明确"此时 k 是第一个非活跃层索引；`kroot = k - 1` 恰好是最后一个活跃层"，与 Fortran 1-based 行为一致；下游用法 `for k in max(kroot, 1):nzg` 保持。
