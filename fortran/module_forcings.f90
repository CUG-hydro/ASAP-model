...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGS(is, ie, js, je, filename, irec, hour, topo, lats, lons &
                           , vels, temp, pres, qair, tempsfc, landmask, topoera5, varpack, request, istart)
      use netcdf

      integer :: is, ie, js, je, i, j, ii, jj, n, istart, hour
      ...
      real, dimension(is:ie, js:je) :: topo, topoera5, lats, lons, vels, temp, pres, qair, tempsfc
      real :: tempint, elev, relh, tempsfcint
...
!Wind velocity
 ...
 filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/WIND/ERA5_wind_speed_'//filename(1:len_trim(filename))
...
!write(6,*)'reading wind forcing from ',filee2obs
         ...
         call readvar(ncid, xdim, ydim, 'ws10', start, count, wind)
...
!Temperature
    ...
    filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/TEMP/ERA5_2m_temperature_'//filename(1:len_trim(filename))
...
!write(6,*)'reading forcing from ',filee2obs
         ...
         call readvar(ncid, xdim, ydim, 't2m', start, count, tempera)
...
!Pressure
    ...
    filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/SFCPRESS/ERA5_surface_pressure_'//filename(1:len_trim(filename))
...
!write(6,*)'reading forcing from ',filee2obs
         ...
         call readvar(ncid, xdim, ydim, 'sp', start, count, press)
...
!Mixing ratio to rh
    ...
    filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/DEWTEMP/ERA5_2m_dewpoint_temperature_'//filename(1:len_trim(filename))
...
!write(6,*)'reading forcing from ',filee2obs
         ...
         call readvar(ncid, xdim, ydim, 'd2m', start, count, dtemp)
...
!Precipitation
   ...
   filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/PRECIP/ERA5_precip_'//filename(1:len_trim(filename))
...
!write(6,*)'reading from ',filee2obs,irec
         ...
         call readvar(ncid, xdim, ydim, 'tp', start, count, rain)

!      rain =  dble(varread)*scale_factor + add_offset

         call readvar(ncid, xdim, ydim, 'sf', start, count, varread)

         rain = max(rain - varread, 0.)*1000. !in mm
...
!Shortwave radiation
    ...
    filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/RADIATION/ERA5_surface_solar_radiation_downwards_'//filename(1:len_trim(filename))
...
!write(6,*)'reading from ',filee2obs,irec
         ...
         call readvar(ncid, xdim, ydim, 'ssrd', start, count, swdown)
...
!net radiation
    ...
    filee2obs = '/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/RADIATION/ERA5_surface_net_radiation_'//filename(1:len_trim(filename))
...
!write(6,*)'reading from ',filee2obs,irec
         ...
         call readvar(ncid, xdim, ydim, 'sr', start, count, rnetera)
...
!soil levels
 ...
 filee2obs='/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/SOILTEMP/ERA5_soiltemp_'//filename(1:len_trim(filename))
...
!write(6,*)'reading from ',filee2obs,irec
         ...
         call readvar(ncid, xdim, ydim, 'stl1', start, count, st1)
!           st1 = dble(varread)*scale_factor + add_offset
         call readvar(ncid, xdim, ydim, 'stl2', start, count, st2)
!           st2 = dble(varread)*scale_factor + add_offset
         call readvar(ncid, xdim, ydim, 'stl3', start, count, st3)
!           st3 = dble(varread)*scale_factor + add_offset
         call readvar(ncid, xdim, ydim, 'stl4', start, count, st4)
...
!          gry = ydim - 298 + 1 - (lats(1,j) - xswlat) / dy
            gry = dimeray + (xswlat - lats(is, j))/dy
...
!temp in soil surface layer
                  call gdtost3(varpack(1, 1, hour, 8), dimerax, dimeray, ii, jj, wt1, wt2, wt3, wt4, tempsfcint)

                  vels(i, j) = max(vels(i, j), 0.)
                  temp(i, j) = tempint - 0.0065*(topo(i, j) - topoera5(i, j))
                  tempsfc(i, j) = tempsfcint - 0.0065*(topo(i, j) - topoera5(i, j))
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGSACC(is, ie, js, je, filename, irec, hour, lats, lons &
      ...
      character*200 :: filee2obs
      integer, dimension(is:ie, js:je) :: landmask
      real, dimension(is:ie, js:je) :: lats, lons, pcpgl, rshort, netrad
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGSRNET(is, ie, js, je, filename, irec, hour, lats, lons, rnet, landmask, netrad, req, istart)
      use netcdf
      use interp_lib

      integer :: is, ie, js, je, i, j, n, hour, istart
      integer ::domblocke2obs, request, req(numtasks - 2), stats(MPI_STATUS_SIZE, numtasks - 2)
      character*50 :: filename
      character*200 :: filee2obs
      integer, dimension(is:ie, js:je) :: landmask
      real, dimension(is:ie, js:je) :: lats, lons, rnet
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGSSNOW(is, ie, js, je, filename, irec, hour, lats, lons, snow, landmask, snowera, request, istart)
      use netcdf
      use interp_lib

      integer :: is, ie, js, je, i, j, n, hour, istart, icount
      integer ::domblocke2obs, request, req(numtasks - 2), stats(MPI_STATUS_SIZE, numtasks - 2)
      character*50 :: filename
      character*200 :: filee2obs
      integer, dimension(is:ie, js:je) :: landmask
      real, dimension(is:ie, js:je) :: lats, lons, snow
      real, dimension(dimeralandx, dimeralandy, 0:23) :: snowera
      real, allocatable, dimension(:, :, :) :: varread
      integer :: irec
      real :: dx, dy, grx, gry, xswlat, xswlon
      double precision :: scale_factor, add_offset
      ...
      call MPI_Type_CONTIGUOUS(dimeralandx*dimeralandy*24, MPI_REAL, domblocke2obs, ierr)
      call MPI_Type_commit(domblocke2obs, ierr)

      if (hour .eq. 0) then
         IF (pid .eq. 0) THEN

            allocate (varread(xdimeraland, ydimeraland, 0:23))
...
!Net short wave radiation
    ...
    filee2obs='/mnt/lustre/hsm/nlsas/notape/home/usc/fm/gfnlmeteo/GONZALO/ERA5/SNOWMELT/ERA5LAND_snowmelt_'//filename(1:len_trim(filename))
...
!        snowera(1:620,1:720,1:8) = 1000 * max( dble(varread(2666:3285,747:1466,1:8))*scale_factor + add_offset, 0. ) !in mm
            snowera(1:620, 1:720, 0:23) = 1000*max(varread(2666:3285, 747:1466, 0:23), 0.) !in mm
...
!          gry = ydimeraland - 747 + 1 - (lats(1,j) - xswlat) / dy
            gry = dimeralandy + (xswlat - lats(is, j))/dy
            do i = is, ie
               if (landmask(i, j) .gt. 0) then
!          gry = (lats(i,j) - xswlat) / dy + 1.
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGSSOILT(is, ie, js, je, nzg, filename, irec, hour, lats, lons, icefactor, landmask, topo, topoera &
                                , smoipack, req, istart)
      use netcdf
      use interp_lib

      integer :: is, ie, js, je, i, j, ii, jj, n, nlev, nzg, istart, hour
      integer ::domblocke2obs, request, req(numtasks - 2), stats(MPI_STATUS_SIZE, numtasks - 2)
      character*50 :: filename
      character*200 :: filee2obs
      ...
      integer, dimension(is:ie, js:je) :: landmask
      real, dimension(is:ie, js:je) :: lats, lons, topo, topoera
      integer*1, dimension(is:ie, js:je, nzg - 14:nzg) :: icefactor
      real, allocatable, dimension(:, :, :) :: rain, swdown, rnetera
      integer*2, allocatable, dimension(:, :, :) :: varread
...
!******************************************************************************************************
   subroutine readvar(ncid, xeraread, yeraread, varname, start, count, varread)
      use netcdf

      integer :: xeraread, yeraread
      real, dimension(xeraread, yeraread, 0:23) :: varread
...
!******************************************************************************************************
   subroutine READTOPOERA5(n2, n3, is, ie, js, je, landmask, lats, lons, topoera5)
      use netcdf
      use interp_lib

      integer :: n2, n3, is, ie, js, je, i, j, irec, iun, k, n
      real, dimension(is:ie, js:je) :: lats, lons, topoera5
      character*200 :: filee2obs
      integer, dimension(is:ie, js:je) :: landmask
...
!******************************************************************************************************
   SUBROUTINE READLAI(n2, is, ie, js, je, lats, lons, year, month, day, lai)
      ...
      integer :: n2, is, ie, js, je, irec, year, month, day, currday, firstday
      integer, dimension(nxlai, nylai) :: modislai
      real, dimension(is:ie, js:je) :: lai, lats, lons
...
!******************************************************************************************************
   subroutine READLAICLIM(n2, n3, is, ie, js, je, month, lai)
      ...
      integer :: n2, n3, is, ie, js, je, i, j, irec, iun, k, n, month
      real, dimension(is:ie, js:je) :: lai
      ...
      elseif (pid .lt. numtasks - 1) then
         call MPI_irecv(lai(is, js), 1, domblock(pid), 0, 53, MPI_COMM_WORLD, request, ierr)
         call MPI_wait(request, status, ierr)
      end if
...
!******************************************************************************************************
   subroutine READO18CLIM(n2, n3, is, ie, js, je, month, fileO18, o18ratio)
      ...
      integer :: n2, n3, is, ie, js, je, i, j, irec, iun, k, n, month
      real, dimension(is:ie, js:je) :: o18ratio
      ...
      character(len=*) :: fileo18

      if (pid .eq. 0) then

         write (6, *) 'sending O18 to nodes'
         ...
         statusnc = nf90_open(fileo18, 0, ncid)
         if (statusnc .ne. nf90_noerr) call handle_err(statusnc)

         statusnc = nf90_inq_varid(ncid, 'O18', varid)
         if (statusnc .ne. nf90_noerr) call handle_err(statusnc)
...
!      if (statusnc .ne. nf90_noerr) call handle_err(statusnc)
         ...
         varread(1:n2, 1:n3) = (varreadbig(nw:ne, ns:nn)*1.e-3 + 1.)*2005.20*1.e-6

         do n = 1, numtasks - 2
            call MPI_isend(varread(1, 1), 1, arraysection(n), n, 73, MPI_COMM_WORLD, req(n), ierr)
         end do
      ...
      elseif (pid .lt. numtasks - 1) then
         call MPI_irecv(o18ratio(is, js), 1, domblock(pid), 0, 73, MPI_COMM_WORLD, request, ierr)
         call MPI_wait(request, status, ierr)
      end if

   end subroutine readO18clim
!******************************************************************************************************
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGS6h(n2, is, ie, js, je, lats, lons, topo, temp, press, ncount, FILE_NAME_NETCDF)
      use netcdf

      integer :: n2, is, ie, js, je, i, j, ncount, n, ihour
      ...
      real, dimension(is:ie, js:je) :: topo, lats, lons, temp, press
...
!********************************************************************************
   ...
   SUBROUTINE READFORCINGS3h(n2, is, ie, js, je, lats, lons, rad, prep, ncount, FILE_NAME_NETCDF)
      use netcdf
      ...
      real, dimension(is:ie, js:je) :: lats, lons, rad, prep
...
!          write(63,rec=1)((rad(i,j),i=is,ie),j=js,je)
!          write(63,rec=2)((prep(i,j),i=is,ie),j=js,je)
!    close(63)
