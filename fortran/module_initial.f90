...
!**********************************************************************************************
SUBROUTINE INITIALIZE(n2,n3,is,ie,js,je,nzg,freedrain,slz,dz,soiltextures,wtd,smoi,smoieq &
                      ,fdepth,topo,landmask,deltat,area)
use module_parallel
use module_rootdepth
use module_wtable

integer :: n2,n3,is,ie,js,je,nzg,nss_x,nee_x,nss_y,nee_y,i,j,iwtd,nsoil,nsoil1,k,freedrain
real , dimension(nzg+1) :: slz
real , dimension(nzg) :: dz,vctr4,vctr5,vctr6
integer, dimension(is:ie,js:je) :: landmask
real, dimension(is:ie,js:je) :: wtd,smoiwtd,fdepth,topo,area,qlat,klat
...
integer, dimension(2,is:ie,js:je) :: soiltextures
integer, dimension(nzg) :: soiltxt


   nss_x=is+1
   if(numtasks.eq.1)nss_x=is

   nee_x=ie-1
   if(numtasks.eq.1)nee_x=ie

   nss_y=js+1
   if(numtasks.eq.1)nss_y=js

   nee_y=je-1
   if(numtasks.eq.1)nee_y=je
...
call lateralflow(n2,n3,is,ie,js,je,wtd,qlat,fdepth,topo,landmask,deltat,area,klat)
...
SUBROUTINE EQSOILMOISTUREtheor(is,ie,js,je,nzg,slz,dz,soiltextures,landmask,fdepth,smoieq)
use module_parallel
use module_rootdepth

integer :: is,ie,js,je,nzg,nss_x,nee_x,nss_y,nee_y,i,j,nsoil,nsoil1,k,iter
real , dimension(nzg+1) :: slz
real , dimension(nzg) :: dz,vctr2,vctr4,vctr5,vctr6
integer, dimension(is:ie,js:je) :: landmask
real, dimension(is:ie,js:je) :: fdepth
...
SUBROUTINE EQSOILMOISTURE(is,ie,js,je,nzg,slz,dz,dtll,soiltextures,landmask,smoieq)
use module_parallel
use module_rootdepth
...
!******************************************************************************
SUBROUTINE EQSOILMOISTUREiter(is,ie,js,je,nzg,slz,dz,dtll,soiltxt,landmask,fdepth,smoieq)
use module_parallel
use module_rootdepth

integer :: is,ie,js,je,nzg,nss_x,nee_x,nss_y,nee_y,i,j,nsoil,nsoil1,k,kk,iter,flag,freedrain,itermax
...
if(pid.eq.numtasks-1)write(6,*)'mirar',nss_x,nee_x
