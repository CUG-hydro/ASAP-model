...
"""
function soilfluxes(i::Int, j::Int, nzg::Int, freedrain::Int, dtll::Float64, 
                   slz::Vector{Float64}, dz::Vector{Float64}, soiltxt::Int, 
                   smoiwtd::Float64, transp::Vector{Float64}, transpdeep::Float64,
                   smoi::Vector{Float64}, wtd::Float64, precip::Float64, pet_s::Float64,
                   fdepth::Float64, qlat::Float64, qrf::Float64, flood::Float64,
                   icefactor::Vector{Int8}, smoieq::Vector{Float64}, o18::Vector{Float64},
                   precipo18::Float64, tempsfc::Float64, qlato18::Float64, transpo18::Float64)
    ...
    if iwtd - 1 == nzg
        ...
        cc[nzg] = 0.0
        bb[nzg] = -dz[nzg] / dtll
        rr[nzg] = vt3di[nzg+1] / dtll - smoi[nzg] * dz[nzg] / dtll + transp[nzg] / dtll +
                  min(kfmid[nzg] + diffmid[nzg] * vctr6[nzg] * (smoi[nzg] - smoi[nzg-1]), 0.0)
    else
    ...
    if freedrain != 1
        if iwtd <= 2
            ...
            cc[k] = diffmid[k+1] * vctr6[k+1]
            bb[k] = -(cc[k] + dz[k] / dtll)
            rr[k] = -smoi[k] * dz[k] / dtll - kfmid[k+1] + transp[k] / dtll +
                    min(kfmid[k] + diffmid[k] * vctr6[k] * (smoi[k] - smoi[k-1]), 0.0)
            
            k = iwtd - 2
            aa[k] = 0.0
            cc[k] = 0.0
            bb[k] = -dz[k] / dtll
            rr[k] = -smoi[k] * dz[k] / dtll + 
                    max(-kfmid[k+1] - diffmid[k+1] * vctr6[k+1] * (smoi[k+1] - smoi[k]), 0.0)
        end
    else
        # 重力排水边界
        soil_params = get_soil_params(soiltxt)
    ...
    if smoiold[k] < smoicp
        ...
        if vt3di[k+1] > dsmoi
            ...
            smoiold[k] = smoiold[k] + max(vt3di[k+1], 0.0) / dz[k]
    ...
    transpo18_out = 0.0
    for k in 1:nzg
        transpo18_out += 0.5 * (o18[k] / smoiold[k] + o18ratio[k]) * transp[k]
        if o18[k] < 0.0
    ...
    return et_s, runoff, rech, flux, qrfcorrect, transpo18_out, smoi, o18
end
