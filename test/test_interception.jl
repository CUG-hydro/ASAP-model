...
截留模块测试
"""
module TestInterception

using Test
include("../src/Interception.jl")
using .Interception

function test_interception()
    @testset "截留计算测试" begin
        # 测试基本截留计算
        ...
        @testset "小降水情况" begin
            ...
            deficit = intercepmax - intercepstore  # 0.4 mm
            @test ppdrip ≈ precip - deficit  # -0.395 mm，但逻辑上应该是正值
            @test et_i ≈ min(intercepmax, pet_i)  # 0.5 mm，但受intercepmax限制
            @test new_intercepstore ≈ intercepmax - et_i
        end
